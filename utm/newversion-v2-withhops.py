...
if __name__ == '__main__':
	...
	parser.add_argument('Maxhops',help='Max Number of hops for shortest path',type=int)
...
#	dem=demandslist
	...
	linknormal,failed=normalTraffic(G,dem,copy.deepcopy(linkdictzero),metric,num,maxhops)
	
	temp_node,wct_node,failed_by_node=node_failure(routerlist,copy.deepcopy(failover),copy.deepcopy(linkdictzero),copy.deepcopy(dem),IGPlinks,linknormal,metric,num,maxhops)

	temp_srlg,wct_srlg,failed_by_srlg=srlg_failure(routerlist,copy.deepcopy(failover),copy.deepcopy(linkdictzero),copy.deepcopy(srlgdict),copy.deepcopy(dem),IGPlinks,linknormal,metric,num,maxhops)	
#Writing output file
	...
	with open("ecmp-all-path-wct-allfailures-pythonversion-hopsadded.csv", 'w') as myfile:
